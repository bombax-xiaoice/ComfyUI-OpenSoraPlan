    ...
    def prepare_mask_masked_video(
        ...
        if not use_vae_preprocessed_mask:
            ...
            if torch.cuda.is_available() and device==torch.device('cpu'):
                self.vae.to(torch.device('cuda:0'))
                masked_video = masked_video.to(device=torch.device('cuda:0'), dtype=torch.float16)
            
            masked_video = self.vae.encode(masked_video)
            
            if torch.cuda.is_available() and device==torch.device('cpu'):
                self.vae.to(torch.device('cpu'))
                masked_video = masked_video.to(device=torch.device('cpu'), dtype=torch.float32)
            ...
            if torch.cuda.is_available() and device==torch.device('cpu'):
                self.vae.to(torch.device('cuda:0'))
                masked_video = masked_video.to(device=torch.device('cuda:0'), dtype=torch.float16)
                mask = mask.to(device=torch.device('cuda:0'), dtype=torch.float16)
            ...
            if torch.cuda.is_available() and device==torch.device('cpu'):
                self.vae.to(torch.device('cpu'))
                masked_video = masked_video.to(device=torch.device('cpu'), dtype=torch.float32)
                mask = mask.to(device=torch.device('cpu'), dtype=torch.float32)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if device != self.transformer.device:
            prompt_embeds = prompt_embeds.to(device=self.transformer.device, dtype=torch.float16) if prompt_embeds!=None else None
            prompt_attention_mask = prompt_attention_mask.to(device=self.transformer.device, dtype=torch.float16) if prompt_attention_mask!=None else None
            negative_prompt_embeds = negative_prompt_embeds.to(device=self.transformer.device, dtype=torch.float16) if negative_prompt_embeds!=None else None
            negative_prompt_attention_mask = negative_prompt_attention_mask.to(device=self.transformer.device, dtype=torch.float16) if negative_prompt_attention_mask!=None else None
