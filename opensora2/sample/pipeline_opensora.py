    ...
    def __call__(
        ...
        **kwargs,
        ...
        if device != self.transformer.device:
            prompt_embeds = prompt_embeds.to(device=self.transformer.device, dtype=torch.float16)
            prompt_attention_mask = prompt_attention_mask.to(device=self.transformer.device, dtype=torch.float16)
            negative_prompt_embeds = negative_prompt_embeds.to(device=self.transformer.device, dtype=torch.float16) if negative_prompt else None
            negative_prompt_attention_mask = negative_prompt_attention_mask.to(device=self.transformer.device, dtype=torch.float16) if negative_prompt_attention_mask else None
            device = self.transformer.device
            print('promot cpu->gpu')
        else:
            print('prompt keep')
