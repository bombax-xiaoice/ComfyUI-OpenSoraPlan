...
from opensora2.models.diffusion.opensora.modules import OverlapPatchEmbed3D, OverlapPatchEmbed2D, PatchEmbed2D, BasicTransformerBlock
from opensora2.utils.utils import to_2tuple
try:
    import torch_npu
    from opensora2.npu_config import npu_config
    from opensora2.acceleration.parallel_states import get_sequence_parallel_state, hccl_info
except:
    torch_npu = None
    npu_config = None
    from opensora2.utils.parallel_states import get_sequence_parallel_state, nccl_info
...
class OpenSoraT2V(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if torch.cuda.is_available() and self.device == cpu_device:
            hidden_states_vid = hidden_states_vid.to(device=cuda_device, dtype=torch.float16) if hidden_states_vid!=None else None
            hidden_states_img = hidden_states_img.to(device=cuda_device, dtype=torch.float16) if hidden_states_img!=None else None
            attention_mask_vid = attention_mask_vid.to(device=cuda_device, dtype=torch.float16) if attention_mask_vid!=None else None
            attention_mask_img = attention_mask_img.to(device=cuda_device, dtype=torch.float16) if attention_mask_img!=None else None
            encoder_hidden_states_img = encoder_hidden_states_img.to(device=cuda_device, dtype=torch.float16) if encoder_hidden_states_img!=None else None
            encoder_hidden_states_vid = encoder_hidden_states_vid.to(device=cuda_device, dtype=torch.float16) if encoder_hidden_states_vid!=None else None
            encoder_attention_mask_img = encoder_attention_mask_img.to(device=cuda_device, dtype=torch.float16) if encoder_attention_mask_img!=None else None
            encoder_attention_mask_vid = encoder_attention_mask_vid.to(device=cuda_device, dtype=torch.float16) if encoder_attention_mask_vid!=None else None
            timestep_img = timestep_img.to(device=cuda_device, dtype=torch.float16) if timestep_img!=None else None
            timestep_vid = timestep_vid.to(device=cuda_device, dtype=torch.float16) if timestep_vid!=None else None
            class_labels = class_labels.to(device=cuda_device, dtype=torch.float16) if class_labels!=None else None

        for i, block in enumerate(self.transformer_blocks):
            if torch.cuda.is_available() and self.device == cpu_device:
                block = block.to(device=cuda_device)
            ...
            if torch.cuda.is_available() and self.device == cpu_device:
                block = block.to(device=cpu_device)
        #end for block in self.transformer_blocks
      
        if torch.cuda.is_available() and self.device == cpu_device:
            hidden_states_img = hidden_states_img.to(device=cpu_device, dtype=torch.float32) if hidden_states_img!=None else None
            hidden_states_vid = hidden_states_vid.to(device=cpu_device, dtype=torch.float32) if hidden_states_vid!=None else None
            attention_mask_vid = attention_mask_vid.to(device=cpu_device, dtype=torch.float32) if attention_mask_vid!=None else None
            attention_mask_img = attention_mask_img.to(device=cpu_device, dtype=torch.float32) if attention_mask_img!=None else None
            encoder_hidden_states_img = encoder_hidden_states_img.to(device=cpu_device, dtype=torch.float32) if encoder_hidden_states_img!=None else None
            encoder_hidden_states_vid = encoder_hidden_states_vid.to(device=cpu_device, dtype=torch.float32) if encoder_hidden_states_vid!=None else None
            encoder_attention_mask_img = encoder_attention_mask_img.to(device=cpu_device, dtype=torch.float32) if encoder_attention_mask_img!=None else None
            encoder_attention_mask_vid = encoder_attention_mask_vid.to(device=cpu_device, dtype=torch.float32) if encoder_attention_mask_vid!=None else None
            timestep_img = timestep_img.to(device=cpu_device, dtype=torch.float32) if timestep_img!=None else None
            timestep_vid = timestep_vid.to(device=cpu_device, dtype=torch.float32) if timestep_vid!=None else None
            class_labels = class_labels.to(device=cpu_device, dtype=torch.float32) if class_labels!=None else None
    ...
    def _operate_on_patched_inputs(self, hidden_states, encoder_hidden_states, timestep, added_cond_kwargs, batch_size, frame, use_image_num):
        ...
        if self.device == torch.device('cpu'):
            self.pos_embed.to(device=torch.device('cuda:0'))
            hidden_states = hidden_states.to(device=torch.device('cuda:0'), dtype=torch.float16)
        hidden_states_vid, hidden_states_img = self.pos_embed(hidden_states.to(self.dtype), frame)
        if self.device == torch.device('cpu'):
            self.pos_embed.to(device=torch.device('cpu'))
            hidden_states_vid = hidden_states_vid.to(device=torch.device('cpu'), dtype=torch.float32) if hidden_states_vid!=None else None
            hidden_states_img = hidden_states_img.to(device=torch.device('cpu'), dtype=torch.float32) if hidden_states_img!=None else None
        ...
        if self.caption_projection is not None:
            if self.device == torch.device('cpu'):
                self.caption_projection.to(device=torch.device('cuda:0'))
                encoder_hidden_states = encoder_hidden_states.to(device=torch.device('cuda:0'), dtype=torch.float16)
            ...
            if self.device == torch.device('cpu'):
                self.caption_projection.to(device=torch.device('cpu:0'))
                encoder_hidden_states = encoder_hidden_states.to(device=torch.device('cpu:0'), dtype=torch.float32) if encoder_hidden_states!=None else None
    ...
    def _get_output_for_patched_inputs(
        ...
        if self.config.norm_type != "ada_norm_single":
            if self.device == torch.device('cpu'):
                self.transformer_blocks[0].norm1.emb.to(device=torch.device('cuda:0'), dtype=torch.float16)
                self.proj_out_1.to(device=torch.device('cuda:0'), dtype=torch.float16)
                self.norm_out.to(device=torch.device('cuda:0'), dtype=torch.float16)
                self.proj_out_2.to(device=torch.device('cuda:0'), dtype=torch.float16)
                timestep.to(device=torch.device('cuda:0'), dtype=torch.float16) if timestep!=None else None
                class_labels.to(device=torch.device('cuda:0'), dtype=torch.float16) if class_labels!=None else None
                hidden_states = hidden_states.to(device=torch.device('cuda:0'), dtype=torch.float16) if hidden_states!=None else None
            ...
            if self.device == torch.device('cpu'):
                self.transformer_blocks[0].norm1.emb.to(device=torch.device('cpu'), dtype=torch.float32)
                self.proj_out_1.to(device=torch.device('cpu'), dtype=torch.float32)
                self.norm_out.to(device=torch.device('cpu'), dtype=torch.float32)
                self.proj_out_2.to(device=torch.device('cpu'), dtype=torch.float32)
                timestep.to(device=torch.device('cpu'), dtype=torch.float32) if timestep!=None else None
                class_labels.to(device=torch.device('cpu'), dtype=torch.float32) if class_labels!=None else None
                hidden_states = hidden_states.to(device=torch.device('cpu'), dtype=torch.float32) if hidden_states!=None else None
        elif self.config.norm_type == "ada_norm_single":
            if self.device == torch.device('cpu'):
                self.norm_out.to(device=torch.device('cuda:0'), dtype=torch.float16)
                self.proj_out.to(device=torch.device('cuda:0'), dtype=torch.float16)
                hidden_states = hidden_states.to(device=torch.device('cuda:0'), dtype=torch.float16) if hidden_states!=None else None
            shift, scale = (self.scale_shift_table[None] + embedded_timestep[:, None]).chunk(2, dim=1)
            hidden_states = self.norm_out(hidden_states)
            # Modulation
            hidden_states = hidden_states * (1 + scale.to(device=torch.device('cuda:0'),dtype=torch.float16)) + shift.to(device=torch.device('cuda:0'),dtype=torch.float16)
            hidden_states = self.proj_out(hidden_states)
            hidden_states = hidden_states.squeeze(1)
            if self.device == torch.device('cpu'):
                self.norm_out.to(device=torch.device('cpu'), dtype=torch.float32)
                self.proj_out.to(device=torch.device('cpu'), dtype=torch.float32)
...
if __name__ == '__main__':
    from opensora2.models.causalvideovae import ae_channel_config, ae_stride_config
    from opensora2.models.causalvideovae import getae, getae_wrapper
    from opensora2.models.causalvideovae import CausalVAEModelWrapper
