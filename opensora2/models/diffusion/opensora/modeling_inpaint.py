...
class OpenSoraInpaint(OpenSoraT2V):
    ...
    def _operate_on_patched_inputs(self, hidden_states, encoder_hidden_states, timestep, added_cond_kwargs, batch_size, frame, use_image_num, vip_tokens=None):
        ...
        if self.device == torch.device('cpu'):
            ...
            hidden_states_masked_vid = hidden_states_masked_vid.to(device=torch.device('cuda:0'), dtype=torch.float16) if hidden_states_masked_vid!=None else None
            hidden_states_mask = hidden_states_mask.to(device=torch.device('cuda:0'), dtype=torch.float16) if hidden_states_mask!=None else None
            self.pos_embed.to(device=torch.device('cuda:0'), dtype=torch.float16)
            self.pos_embed_masked_video[0].to(device=torch.device('cuda:0'), dtype=torch.float16)
            self.pos_embed_masked_video[1].to(device=torch.device('cuda:0'), dtype=torch.float16)
            self.pos_embed_mask[0].to(device=torch.device('cuda:0'), dtype=torch.float16)
            self.pos_embed_mask[1].to(device=torch.device('cuda:0'), dtype=torch.float16)
        ...
        if self.device == torch.device('cpu'):
            ...
            hidden_states_masked_vid = hidden_states_masked_vid.to(device=torch.device('cpu'), dtype=torch.float32) if hidden_states_masked_vid!=None else None
            hidden_states_mask = hidden_states_mask.to(device=torch.device('cpu'), dtype=torch.float32) if hidden_states_mask!=None else None
            self.pos_embed.to(device=torch.device('cpu'), dtype=torch.float32)
            self.pos_embed_masked_video[0].to(device=torch.device('cpu'), dtype=torch.float32)
            self.pos_embed_masked_video[1].to(device=torch.device('cpu'), dtype=torch.float32)
            self.pos_embed_mask[0].to(device=torch.device('cpu'), dtype=torch.float32)
            self.pos_embed_mask[1].to(device=torch.device('cpu'), dtype=torch.float32)
        ...
        if self.adaln_single is not None:
            ...
            if self.device == torch.device('cpu'):
                timestep = timestep.to(device=torch.device('cuda:0'), dtype=torch.float16) if timestep!=None else None
                self.adaln_single.to(device=torch.device('cuda:0'), dtype=torch.float16)
            ...
            if self.device == torch.device('cpu'):
                timestep = timestep.to(device=torch.device('cpu'), dtype=torch.float32) if timestep!=None else None
                embedded_timestep = embedded_timestep.to(device=torch.device('cpu'), dtype=torch.float32) if embedded_timestep!=None else None
                self.adaln_single.to(device=torch.device('cpu'), dtype=torch.float32)
        ...
        if self.device == torch.device('cpu'):
            ...
            self.caption_projection.to(device=torch.device('cuda:0'), dtype=torch.float16)
        ...
        if self.device == torch.device('cpu'):
            encoder_hidden_states = encoder_hidden_states.to(device=torch.device('cpu'), dtype=torch.float32) if encoder_hidden_states!=None else None
            encoder_hidden_states_img = encoder_hidden_states_img.to(device=torch.device('cpu'), dtype=torch.float32) if encoder_hidden_states_img!=None else None
            encoder_hidden_states_vid = encoder_hidden_states_vid.to(device=torch.device('cpu'), dtype=torch.float32) if encoder_hidden_states_vid!=None else None
            self.caption_projection.to(device=torch.device('cpu'), dtype=torch.float32)
