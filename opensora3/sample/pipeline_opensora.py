...
class OpenSoraPipelineOutput(BaseOutput):
    images: Union[List[torch.FloatTensor], np.ndarray]
...
class OpenSoraPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        device = device or self.text_encoder.device if self.text_encoder else torch.device('cpu')
        ...
        if device != self.transformer.device:
            prompt_embeds = prompt_embeds.to(device=self.transformer.device, dtype=torch.bfloat16)
            prompt_attention_mask = prompt_attention_mask.to(device=self.transformer.device, dtype=torch.bfloat16)
