...
from opensora3.sample.pipeline_opensora import OpenSoraPipeline, OpenSoraPipelineOutput, rescale_noise_cfg
from opensora3.dataset.transform import CenterCropResizeVideo, SpatialStrideCropVideo,ToTensorAfterResize, maxhwresize
from opensora3.utils.mask_utils import MaskProcessor, MaskCompressor, GaussianNoiseAdder, MaskType, STR_TO_TYPE, TYPE_TO_STR
...
class OpenSoraInpaintPipeline(OpenSoraPipeline):
    ...
    def get_masked_pixel_values_mask(
        ...
        if device is None:
            device = getattr(self, '_execution_device', None) or getattr(self, 'device', None) or torch.device('cpu')
        ...
        device = self.vae.vae.device or torch.device('cpu')
        if device==torch.device('cpu') and torch.cuda.is_available():
            self.vae.vae.to(device=torch.device('cuda:0'), dtype=torch.bfloat16)
            masked_pixel_values = masked_pixel_values.to(device=torch.device('cuda:0'), dtype=torch.bfloat16)
        
        masked_pixel_values = self.vae.encode(masked_pixel_values)
        if device==torch.device('cpu') and torch.cuda.is_available():
            self.vae.vae.to(device=torch.device('cpu'), dtype=torch.float32)
            masked_pixel_values = masked_pixel_values.to(device=torch.device('cpu'), dtype=torch.float32)
    ...
    def __call__(
        ...
        enable_temporal_attentions = False, #compatibility
        num_images_per_prompt = 1, #compatibility
        mask_feature = True, #compatibility
        callback: Optional[Callable[[int, int, torch.FloatTensor], None]] = None, #compatibility
        callback_steps: int = 1, #compatibility
        ...
        if callback_on_step_end==None and callback != None:
            callback_on_step_end = lambda s,i,t,d:{"pbar":callback(i,t,d[callback_on_step_end_tensor_inputs[0] if callback_on_step_end_tensor_inputs else "latents"])}
        ...
        device = self.text_encoder.device if self.text_encoder else torch.device('cpu')
        ...
        if device != self.transformer.device:
            prompt_embeds = prompt_embeds.to(device=self.transformer.device, dtype=torch.bfloat16) if prompt_embeds!=None else None
            prompt_attention_mask = prompt_attention_mask.to(device=self.transformer.device, dtype=torch.bfloat16) if prompt_attention_mask!=None else None
            negative_prompt_embeds = negative_prompt_embeds.to(device=self.transformer.device, dtype=torch.bfloat16) if negative_prompt_embeds!=None else None
            negative_prompt_attention_mask = negative_prompt_attention_mask.to(device=self.transformer.device, dtype=torch.bfloat16) if negative_prompt_attention_mask!=None else None
        ...
        if not output_type == "latent" and not output_type == "latents":
        ...
        return OpenSoraPipelineOutput(images=videos)
