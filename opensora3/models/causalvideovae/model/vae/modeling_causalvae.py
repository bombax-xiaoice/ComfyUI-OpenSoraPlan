...
class CausalVAEModel(VideoBaseAE):
    ...
    def enable_tiling(self, use_tiling: bool = True, tile: int = 64, context: int = 5):
        self.use_tiling = use_tiling
        self.tile_latent_min_size = abs(tile)
        self.tile_latent_min_size_t = abs(context) + 1
        self.tile_sample_min_size = abs(tile) * (2 ** (len(self.hidden_size_mult) - 1)) * 1000 if tile > 0 else abs(tile) * (2 ** (len(self.hidden_size_mult) - 1))
        self.tile_sample_min_size_t = (abs(context)-1) * 8 + 1 if context > 0 else (abs(context)-1) * 4 + 1
