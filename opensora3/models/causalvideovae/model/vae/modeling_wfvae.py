...
class WFVAEModel(VideoBaseAE):
    ...
    def tile_encode(self, x):
        ...
        for idx, (start, end) in enumerate(start_end):
            self._set_first_chunk(idx == 0 or self.tile_latent_min_size > 0) #always use first chunk mode for 2D tile
            chunk = x[:, :, start:end, :, :]

            if self.tile_latent_min_size <= 0: #disable 2D tile
                chunk = self.encoder(chunk)[0]
                if self.use_quant_layer:
                    chunk = self.encoder(chunk)
            else:
                chunk = self.tiled_encode2d(chunk, idx==0, return_moments=True)
    ...
    def tile_decode(self, x):
        ...
        for idx, (start, end) in enumerate(start_end):
            ...
            if self.tile_latent_min_size <= 0: #disable 2D tile
                ...
                chunk = self.tiled_decode2d(chunk, idx==0)
    ...
    def tiled_encode2d(self, x, is_first_chunk=False, return_moments=False):
        overlap_size = int(self.tile_sample_min_size * (1 - self.tile_overlap_factor))
        blend_extent = int(self.tile_latent_min_size * self.tile_overlap_factor)
        row_limit = self.tile_latent_min_size - blend_extent
        #warmup
        tile = x[:,:,:,:self.tile_sample_min_size,:self.tile_sample_min_size]
        tile = self.encoder(tile)[0]
        if self.use_quant_layer:
            tile = self.quant_conv(tile)
        #padding
        xe = torch.nn.functional.pad(x, pad=(0, self.tile_sample_min_size, 0, self.tile_sample_min_size, 0, 0), mode='reflect')
        rows = []
        ...
        for i, row in enumerate(rows):
            ...
            result_rows.append(torch.cat(result_row, dim=4)[:,:,:,:,:x.shape[4]*self.tile_latent_min_size//self.tile_sample_min_size])
        moments = torch.cat(result_rows, dim=3)[:,:,:,:x.shape[3]*self.tile_latent_min_size//self.tile_sample_min_size,:]
        if return_moments:
            return moments
        posterior = DiagonalGaussianDistribution(moments)
        return posterior

    def tiled_decode2d(self, z, is_first_chunk=False):
        overlap_size = int(self.tile_latent_min_size * (1 - self.tile_overlap_factor))
        blend_extent = int(self.tile_sample_min_size * self.tile_overlap_factor)
        row_limit = self.tile_sample_min_size - blend_extent
        #warmup
        tile = z[:,:,:,:self.tile_latent_min_size,:self.tile_latent_min_size]
        if self.use_quant_layer:
            tile = self.post_quant_conv(tile)    
        tile = self.decoder(tile)[0]
        #padding
        ze = torch.nn.functional.pad(z, pad=(0, self.tile_latent_min_size, 0, self.tile_latent_min_size, 0, 0), mode='reflect')
        rows = []
        ...
        for i, row in enumerate(rows):
            ...
            for j, tile in enumerate(row):
                ...
                if j > 0:
                    tile = self.blend_h(row[j - 1], tile, blend_extent)
                result_row.append(tile[:, :, :, :row_limit, :row_limit])
            result_rows.append(torch.cat(result_row, dim=4)[:,:,:,:,:z.shape[4]*self.tile_sample_min_size//self.tile_latent_min_size])
        dec = torch.cat(result_rows, dim=3)[:,:,:,:z.shape[3]*self.tile_sample_min_size//self.tile_latent_min_size,:]
        return dec
    ...
    def enable_tiling(self, use_tiling: bool = True, tile: int = 32, context: int = 4):
        self._set_causal_cached(use_tiling==False or tile <= 0) #disable cache for 2D tile
        self.use_tiling = use_tiling
        self.t_chunk_dec = context
        self.t_chunk_enc = context * 4
        self.tile_latent_min_size = tile
        self.tile_sample_min_size = tile * 8
