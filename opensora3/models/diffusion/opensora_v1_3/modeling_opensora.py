...
from opensora3.models.diffusion.opensora_v1_3.modules import BasicTransformerBlock, Attention
from opensora3.models.diffusion.common import PatchEmbed2D
from opensora3.utils.utils import to_2tuple
try:
    import torch_npu
    from opensora3.npu_config import npu_config
    from opensora3.acceleration.parallel_states import get_sequence_parallel_state, hccl_info
except:
    torch_npu = None
    npu_config = None
    from opensora3.utils.parallel_states import get_sequence_parallel_state, nccl_info
...
class OpenSoraT2V_v1_3(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs, 
        ...
        if torch.cuda.is_available() and self.device == cpu_device:
            hidden_states = hidden_states.to(device=cuda_device, dtype=torch.bfloat16) if hidden_states!=None else None
            encoder_hidden_states = encoder_hidden_states.to(device=cuda_device, dtype=torch.bfloat16) if encoder_hidden_states!=None else None
            timestep = timestep.to(device=cuda_device, dtype=torch.bfloat16) if timestep!=None else None
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if torch.cuda.is_available() and self.device == cpu_device:
                ...
                attention_mask = attention_mask.to(device=cuda_device, dtype=torch.bfloat16) if attention_mask!=None else None
                encoder_attention_mask = encoder_attention_mask.to(device=cuda_device, dtype=torch.bfloat16) if encoder_attention_mask!=None else None
            ...
            if torch.cuda.is_available() and self.device == cpu_device:
                ...
                attention_mask = attention_mask.to(device=cpu_device, dtype=torch.float32) if attention_mask!=None else None
                encoder_attention_mask = encoder_attention_mask.to(device=cpu_device, dtype=torch.float32) if encoder_attention_mask!=None else None
        #end for block in self.transformer_blocks
        if torch.cuda.is_available() and self.device == cpu_device:
            hidden_states = hidden_states.to(device=cpu_device, dtype=torch.float32) if hidden_states!=None else None
            encoder_hidden_states = encoder_hidden_states.to(device=cpu_device, dtype=torch.float32) if encoder_hidden_states!=None else None
            timestep = timestep.to(device=cpu_device, dtype=torch.float32) if timestep!=None else None
    ...
    def _operate_on_patched_inputs(self, hidden_states, encoder_hidden_states, timestep, batch_size, frame):
        if torch.cuda.is_available() and self.device == torch.device('cpu'):
            ...
            hidden_states = hidden_states.to(device=torch.device('cuda:0'), dtype=torch.bfloat16) if hidden_states!=None else None
        ...
        if torch.cuda.is_available() and self.device == torch.device('cpu'):
            ...
            encoder_hidden_states = encoder_hidden_states.to(device=torch.device('cuda:0'), dtype=torch.bfloat16)
        
        encoder_hidden_states = self.caption_projection(encoder_hidden_states)  # b, 1, l, d or b, 1, l, d

        if torch.cuda.is_available() and self.device == torch.device('cpu'):
            self.caption_projection.to(device=torch.device('cpu'))
            encoder_hidden_states = encoder_hidden_states.to(device=torch.device('cpu'), dtype=torch.float32)
    ...
    def _get_output_for_patched_inputs(
        ...
        if torch.cuda.is_available() and self.device == torch.device('cpu'):
            self.norm_out.to(device=torch.device('cuda:0'), dtype=torch.bfloat16)
            self.proj_out.to(device=torch.device('cuda:0'), dtype=torch.bfloat16)
            hidden_states = hidden_states.to(device=torch.device('cuda:0'), dtype=torch.bfloat16) if hidden_states!=None else None
        
        shift, scale = (self.scale_shift_table[None] + embedded_timestep[:, None]).chunk(2, dim=1)
        hidden_states = self.norm_out(hidden_states)
        # Modulation
        hidden_states = hidden_states * (1 + scale.to(device=torch.device('cuda:0'),dtype=torch.bfloat16)) + shift.to(device=torch.device('cuda:0'),dtype=torch.bfloat16)
...
if __name__ == '__main__':
    from opensora3.models.causalvideovae import ae_stride_config, ae_channel_config
    from opensora3.models.causalvideovae import ae_norm, ae_denorm
    from opensora3.models import CausalVAEModelWrapper
