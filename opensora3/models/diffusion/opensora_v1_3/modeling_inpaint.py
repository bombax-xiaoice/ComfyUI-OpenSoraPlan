...
from opensora3.models.diffusion.opensora_v1_3.modeling_opensora import OpenSoraT2V_v1_3 as OpenSoraT2V
...
class OpenSoraInpaint_v1_3(OpenSoraT2V):
    ...
    def _operate_on_patched_inputs(self, hidden_states, encoder_hidden_states, timestep, batch_size, frame):
        ...
        if torch.cuda.is_available() and self.device == torch.device('cpu'):
            ...
            self.pos_embed_masked_hidden_states.to(device=torch.device('cuda:0'))
            self.pos_embed_mask.to(device=torch.device('cuda:0'))
        ...
        if torch.cuda.is_available() and self.device == torch.device('cpu'):
            ...
            self.pos_embed_masked_hidden_states.to(device=torch.device('cpu'))
            self.pos_embed_mask.to(device=torch.device('cpu'))
