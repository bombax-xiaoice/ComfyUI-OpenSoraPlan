...
class VideoGenPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if self.text_encoder.device != self.transformer.device:
            ...
            negative_prompt_embeds = negative_prompt_embeds.to(device=self.transformer.device, dtype=torch.float16)
        ...
        if not output_type == 'latents':
            ...
            video = latents
            return VideoPipelineOutput(images=video)
