...
from opensora1.models.diffusion.utils.pos_embed import get_1d_sincos_pos_embed, PositionGetter1D, PositionGetter2D
from opensora1.models.diffusion.latte.modules import PatchEmbed, BasicTransformerBlock, BasicTransformerBlock_, AdaLayerNormSingle, \
        ...
        for i, (spatial_block, temp_block) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks)):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if torch.cuda.is_available() and self.device == cpu_device:
                    spatial_block = spatial_block.to(device = cuda_device)
                    hidden_states = hidden_states.to(device = cuda_device) if hidden_states!=None else None
                    if i >= self.num_layers // 2:
                        attention_mask_compress = attention_mask_compress.to(device = cuda_device) if attention_mask_compress!=None else None
                    else:
                        attention_mask = attention_mask.to(device = cuda_device) if attention_mask!=None else None
                    encoder_hidden_states_spatial = encoder_hidden_states_spatial.to(device= cuda_device) if encoder_hidden_states_spatial!=None else None
                    encoder_attention_mask = encoder_attention_mask.to(device = cuda_device) if encoder_attention_mask!=None else None
                    timestep_spatial = timestep_spatial.to(device = cuda_device) if timestep_spatial!=None else None
                    class_labels = class_labels.to(device = cuda_device) if class_labels!=None else None
                    pos_hw = pos_hw.to(device = cuda_device) if pos_hw!=None else None
                ...
                if torch.cuda.is_available() and self.device == cpu_device:
                    spatial_block = spatial_block.to(device = cpu_device)
                    hidden_states = hidden_states.to(device = cpu_device) if hidden_states!=None else None
                    if i >= self.num_layers // 2:
                        attention_mask_compress = attention_mask_compress.to(device = cpu_device) if attention_mask_compress!=None else None
                    else:
                        attention_mask = attention_mask.to(device = cpu_device) if attention_mask!=None else None
                    encoder_hidden_states_spatial = encoder_hidden_states_spatial.to(device= cpu_device) if encoder_hidden_states_spatial!=None else None
                    encoder_attention_mask = encoder_attention_mask.to(device = cpu_device) if encoder_attention_mask!=None else None
                    timestep_spatial = timestep_spatial.to(device = cpu_device) if timestep_spatial!=None else None
                    class_labels = class_labels.to(device = cpu_device) if class_labels!=None else None
                    pos_hw = pos_hw.to(device = cpu_device) if pos_hw!=None else None
                ...
                if enable_temporal_attentions:
                    ...
                    if use_image_num != 0 and self.training:
                        ...
                        if torch.cuda.is_available() and self.device == cpu_device:
                            temp_block = temp_block.to(device = cuda_device)
                            hidden_states = hidden_states.to(device = cuda_device) if hidden_states!=None else None
                            timestep_temp = timestep_temp.to(device = cuda_device) if timestep_temp!=None else None
                            class_labels = class_labels.to(device = cuda_device) if class_labels!=None else None
                            pos_t = pos_t.to(device = cuda_device) if pos_t!=None else None
                        ...
                        if torch.cuda.is_available() and self.device == cpu_device:
                            temp_block = temp_block.to(device = cpu_device)
                            hidden_states = hidden_states.to(device = cpu_device) if hidden_states!=None else None
                            timestep_temp = timestep_temp.to(device = cpu_device) if timestep_temp!=None else None
                            class_labels = class_labels.to(device = cpu_device) if class_labels!=None else None
                            pos_t = pos_t.to(device = cpu_device) if pos_t!=None else None
...
if __name__ == '__main__':
    from opensora1.models.ae import ae_channel_config, ae_stride_config
    from opensora1.models.ae import getae, getae_wrapper
