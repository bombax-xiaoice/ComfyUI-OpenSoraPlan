...
class CausalVAEModel(VideoBaseAE_PL):
    ...
    def __init__(
        ...
        if loss_type.startswith('opensora.'):
            loss_type = "opensora1." + loss_type[len('opensora.'):]
    ...
    def enable_tiling(self, use_tiling: bool = True, tile: int = 32, context: int = 16):
        self.use_tiling = use_tiling
        self.tile_latent_min_size = tile #int(self.tile_sample_min_size / (2 ** (len(hidden_size_mult) - 1)))
        t_down_ratio = [i for i in self.encoder_temporal_downsample if len(i) > 0]
        self.tile_latent_min_size_t = context + 1 #int((self.tile_sample_min_size_t-1) / (2 ** len(t_down_ratio))) + 1
        self.tile_sample_min_size = int(self.tile_latent_min_size *  (2 ** (len(self.hidden_size_mult) - 1)))
        self.tile_sample_min_size_t = int(context * (2 ** len(t_down_ratio))) + 1
        #print('tile='+str(self.tile_latent_min_size)+' tile_sample='+str(self.tile_sample_min_size)+' context='+str(self.tile_latent_min_size_t)+' context_sample='+str(self.tile_sample_min_size_t))
